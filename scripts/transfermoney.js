document.getElementById('transferbtn').addEventListener('click', function(){
    const accountNumber = document.getElementById('transferacountnumber')
    ...
    if(parseInt(amount.value) <= parseInt(balance.innerText)){
        ...
        alert(`৳${amount.value} is transfer successfully to ${accountNumber.value} ${new Date().toLocaleString()}`)
            const newTrans = document.createElement('div')
    newTrans.innerHTML = `
    <div class="allTrans w-full h-[81px] bg-white mt-4 rounded-2xl shadow">
            <h1 class="font-bold text-[25px] ml-4 pt-2">Money transferred Successfully</h1>
